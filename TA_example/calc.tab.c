...
{
  "$end", "error", "$undefined", "RET", "NUMBER", "ADDOP", "MULOP",
  "LPAREN", "RPAREN", "$accept", "top", "line", "expr", "term", "factor", YY_NULLPTR
};
#endif
...
#line 51 "calc.y"
{
    switch ((yyvsp[-1].op)) {
    case '*': (yyval.num) = (yyvsp[-2].num) * (yyvsp[0].num); break;
    case '/': (yyval.num) = (yyvsp[-2].num) / (yyvsp[0].num); break; // 想想看，这里会出什么问题？
    }
}
