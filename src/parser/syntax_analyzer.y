...
%start program
%token <node> INTEGER FLOATPOINT IDENTIFIER ARRAY
%token <node> ADD SUB MUL DIV LT LTE GT GTE EQ NEQ ASSIN SEMICOLON COMMA LPARENTHESE RPARENTHESE LBRACE RBRACE LBRACKET RBRACKET
%token <node> ELSE IF INT FLOAT RETURN VOID WHILE
%token <node> EOL ERROR
%type <node> id type-specifier relop addop mulop
