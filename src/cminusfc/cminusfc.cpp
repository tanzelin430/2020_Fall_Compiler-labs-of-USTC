...
void print_help(std::string exe_name) {
    std::cout << "Usage: " << exe_name <<
        " [ -h | --help ] [ -o <target-file> ] [ -emit-llvm ] [-mem2reg] [-loop-search] [-loop-inv-hoist] [-const-propagation] [-active-vars] [-analyze] [-tail-recursion] <input-file>" << std::endl;
}
...
int main(int argc, char **argv) {
    ...
    for (int i = 1;i < argc;++i) {
        if (argv[i] == "-h"s || argv[i] == "--help"s) {
            ...
            activevars = true;
        } else if(argv[i] == "-tail-recursion"s){
            tail_recursion = true;
        } else {
            if (input_path.empty()) {
                input_path = argv[i];
    ...
    if(tail_recursion)
    {
        PM.add_pass<TailRecurtion>(true);
    }
    PM.run();
